...
def black_patch(img, annot):
	...
	return person_exist, mask_areas
	# return masked image
...
if __name__ == '__main__':
	...
	save_dir = os.path.join(sys.path[0], '../dataset/mask_noresize_annot')
	...
	#dir to move annotations
	annot_mv_dir = os.path.join(save_dir, 'annotations')
	create_dir(annot_mv_dir)
	...
	# new annot dir
	new_annot_dir = os.path.join(sys.path[0], '../dataset/mask_noresize_annot/new_annotations')
	...
	for ix, (img_file, annot_file) in enumerate(zip(img_list, annot_list)):
		...
		person_exist, mask_areas = black_patch(img, annot)
		...
		if person_exist:
			if is_test:
				...
				txt_f_test.write("{}\n".format(os.path.basename(img_file).split('.jpg')[0]))
				voc._modify_annot(annot_file, mask_areas, new_annot_dir)
