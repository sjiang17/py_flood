...
model_file = '/fldata/pytorch-model/faster_rcnn_1_10_625.pth'
model = torch.load(model_file)

cp_file = '/fldata/pytorch-model/vgg16_faster_rcnn_iter_1190000.pth'
...
cp.pop('rpn_net.bias')
cp['RCNN_rpn.RPN_Conv.weight'] = cp['rpn_net.weight']
...
for i in [0, 2, 5, 7, 10, 12, 14, 17, 19, 21, 24, 26, 28]:
	...
	cp.pop('vgg.features.{}.bias'.format(str(i)))

model['model'] = cp

torch.save(model, '/fldata/pytorch-model/faster_rcnn_vgg16_coco-jwy.pth')

file_name = '/fldata/pytorch-model/faster_rcnn_vgg16_coco-jwy.pth'
